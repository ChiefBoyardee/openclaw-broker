...
RUNNER_ENV="${RUNNER_ENV:-$REPO_ROOT/runner/runner.env}"
...
echo ""
echo "Run the runner: cd $REPO_ROOT && RUNNER_ENV=$RUNNER_ENV ./runner/start.sh"
echo "Or foreground: cd $REPO_ROOT && export \$(grep -v '^#' \"$RUNNER_ENV\" | xargs) && python runner/runner.py"
